...
export const sendEmail = async (email: string, otp: string, subjectLine?: string, titleText?: string) => {
  try {
    const finalSubject = subjectLine || "MedFlow Email Verification OTP";
    const finalTitleText = titleText || "Verify Your Email Address";
    const mailOptions = {
      from: `"MedFlow" <${process.env.SENDER_EMAIL || "main@medflow.in"}>`,
      to: email,
      subject: finalSubject,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
          <div style="background-color: #0066CC; padding: 20px; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">MedFlow Registration</h1>
          </div>
          ...
          <div style="background-color: #f9fafb; padding: 15px; text-align: center; border-top: 1px solid #eeeeee;">
            <p style="color: #888888; font-size: 12px; margin: 0;">© ${new Date().getFullYear()} MedFlow. All rights reserved.</p>
          </div>
        </div>
      `,
    };
